...
readBuffer = open("corefoverlay.html", 'r')
bssItem = 0
while bssItem < 4:
	# TODO: we should write all the characters up to the next tag opening if they are not included in a bytespan at all. We might check this by looking at previous bytespans, or the offset graph.
	index = bss[bssItem].getStart()
	read.seek(index)
	write.seek(index + graph[index])
	
	readBuffer.seek(bss[bssItem].getEnd()+1 + graph[bss[bssItem].getEnd()])
	buffer = readBuffer.read()
	
	tag = generateTagOpen(bss[bssItem].getCorefId())
	updateGraph(index, len(tag))
	write.write(tag)
	print tag
	
	holder = ""
	while index <= bss[bssItem].getEnd():
		c = read.read(1)
		write.write(c)
		holder += c
		index += 1
